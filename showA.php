...
while ($rows=mysqli_fetch_array($resc)) {
    $nbrc=$rows['nbrc'];
    for ($i=1;$i <= $nbrc; ++$i) {
  ...
  <form method="POST">
    ...
    <div class="d-flex justify-content-between align-items-center">
      <h2>HACO Shipyard</h2><br>
            ...
            <button><a href="home.php" >Home</a></button></div>
...
<table id="tbl_exporttable" class="table" cellspacing="20">
  ...
  <tbody>
      <?php 
      $view = "SELECT * FROM persone p,social s,hacodata h WHERE p.Mat=s.Mat and p.Mat=h.Mat and status='Active'";
